...
@Component({
  selector: 'app-login-homier',
  templateUrl: './login-homier.component.html',
  styleUrls: ['./login-homier.component.css']
})
export class LoginHomierComponent implements OnInit {
  ...
  login(loginForm) {
    ...
    let returnData = this.loginService.checkAgentHomierLogin(loginForm.value)
    .subscribe(response => {
  ...
  loginSuccess(token) {
   ...
   this.router.navigate([this.urltoRedirect || '/homier']);
